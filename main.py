...
def guarded_handler(event, context):
    ...
    emails = "rpofuk@gmail.com"

    dns_plugin = 'dns-route53'
    s3_bucket = 'localdevhub'
    s3_prefix = 'localdevhub'
    s3_region = 'eu-west-1'


    keys = s3_client.list_objects_v2(Bucket=s3_bucket, Prefix=s3_prefix)
    
    keys = [i['Key'].split('/')[1] for i in keys['Contents']]
    keys = list(set(keys))
    ...
    for i in keys:
        ...
        domains='*.' + i + '.localdevhub.com'
        rm_tmp_dir()
        obtain_certs(emails, domains, dns_plugin)
        upload_certs(s3_bucket, s3_prefix + '/' + i, s3_region)
